...
cleanmap = field+'.'+ifband+'.'+sideband+'.sel.clean.fits'
if_success = False
try:
...
write_to_file('axis_FWHM.sel.txt', field, str(major_axis)+' '+str(minor_axis))
write_to_file('flux.sel.txt', field, total_flux)
write_to_file('SNR.sel.txt', field, SNR)
write_to_file('box.sel.txt', field, box)
