...
#		select='uvrange(0,80)'
        ...
        clean map=$target'.'$track'.'$rx'.'$sideband'.dirty' \
                ...
                out=$target'.'$track'.'$rx'.'$sideband'.10.model' cutoff=0.01 niters=10 options=positive
...
#		select='uvrange(0,80)'
	...
	rm -rf $target'.'$track'.'$rx'.'$sideband'.10.sel.model'
		...
		out=$target'.'$track'.'$rx'.'$sideband'.10.sel.model' cutoff=0.01 niters=10 options=positive options=positive
